package cn.xdev.auth.session.scheduler;

import cn.xdev.auth.session.dao.SessionsMapper;
 ...
 * <p>Date: 14-2-9
 * <p>Version: 1.0
 * 如果会话比较多，会影响性能；可以考虑如分页获取会话并进行验证
 */
public class MySessionValidationScheduler implements SessionValidationScheduler, Runnable {
    ...
    public void run() {
        ...
        int start = 0; //起始记录
        int size = 20; //每页大小
        PageHelper.startPage(start / size + 1, size);
        List<Sessions> sessionList = sessionsMapper.selectByExampleWithBLOBs(new SessionsExample());
        PageInfo page = new PageInfo(sessionList);

        while (sessionList.size() > 0) {
            for (Sessions sessionStr : sessionList) {
                try {
            ...
            start = start + size;
            PageHelper.startPage(start / size + 1, size);
            sessionList = sessionsMapper.selectByExampleWithBLOBs(new SessionsExample());
            new PageInfo(sessionList);
        }
